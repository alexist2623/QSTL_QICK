...
module tb_fir_ddr_1msps_iq64;
    ...
    logic [255:0] expected [0:SHOTS*7-1];
        ...
        if (harness.s_axis_aresetn) begin
            ...
            if (monitor_capture) begin
                if (raw_trigger && !previous_raw_trigger) begin
                    ...
                    // through axis_trigger_sync_v1, then two through DDR V3.
                ...
                if (fir_valid && shot < accepted && cycle >= due_cycle[shot]) begin
                    ...
                    expected[shot*7 + sample_index/2][128*(sample_index%2) +: 128] = fir_data;
                    sample_index++;
    ...
    task automatic run_capture(input int delay_cycles);
        ...
        begin
            ...
            shot = 0;
            sample_index = 0;
            for (int i = 0; i < SHOTS*7; i++) expected[i] = 0;
            ...
            harness.check(harness.completed_count == SHOTS*7,
                "13-sample captures produce seven padded AXI words per trigger");
            for (int i = 0; i < SHOTS*7; i++) begin
    ...
    initial begin
        ...
        if (harness.errors != 0)
            $fatal(1, "FAIL: FIR DDR 1 MSPS IQ64 had %0d errors", harness.errors);
        $display("PASS: FIR DDR 1 MSPS IQ64 continuous phase, 40 kHz triggers, programmable delay, packing and CDC");
        $finish;
    end
endmodule
