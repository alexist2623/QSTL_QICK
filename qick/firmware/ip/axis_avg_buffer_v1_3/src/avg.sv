...
// Maximum supported power-of-two decimation exponent for non-photon I/Q AVG.
...
reg         [3:0]       decim_log2_r;

// Input data.
wire signed	[B-1:0]		din_ii, din_qq;
wire signed [B-1:0]     avg_din_ii, avg_din_qq;
wire                    avg_din_valid;
wire                    decim_valid;
...
assign decim_enabled = (photon_mode_r == 1'b0) && (decim_log2_r != 4'd0);
assign avg_din_valid = (decim_enabled == 1'b1) ? decim_valid : din_valid_i;
assign avg_din_ii = (decim_enabled == 1'b1) ? $signed(decim_din[B-1:0]) : din_ii;
assign avg_din_qq = (decim_enabled == 1'b1) ? $signed(decim_din[2*B-1:B]) : din_qq;
...
avg_decimator_iq
	...
	(
		.rstn           (rstn),
		.clk            (clk),
		.clear_i        ((avg_state == 1'b0) || (decim_enabled == 1'b0)),
	...
	if (~rstn) begin
		...
		case (state)
			...
			AVG_ST:
				if ( START_REG == 1'b0 )
					state <= START_ST;
				else if ( cnt == len_r-1 && avg_din_valid == 1'b1 )
					state <= QOUT_ST;
		...
		else if ( avg_state == 1'b1 && avg_din_valid == 1'b1 ) begin
			// Accumulator counter.
			if ( photon_mode_r == 1'b0 ) begin
				acc_i	<= acc_i + avg_din_ii;
				acc_q	<= acc_q + avg_din_qq;
			end
			// Rising edge counter.
