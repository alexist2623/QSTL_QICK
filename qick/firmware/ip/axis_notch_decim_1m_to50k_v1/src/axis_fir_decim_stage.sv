...
// Two-lane direct-form FIR decimation stage using the same implementation
// method as axis_fir_decim_300to1_v1.
//
// Arithmetic:
//   - two signed 16-bit lanes,
//   - signed 18-bit Q1.17 coefficients,
//   - signed 48-bit accumulation,
//   - round half away from zero before shifting by FIR_COEFF_FRAC_BITS,
...
module axis_fir_decim_stage #(
    ...
    function automatic logic signed [OUT_WIDTH-1:0] round_saturate(
        ...
        begin
            round_offset = ({{(ACC_WIDTH-1){1'b0}}, 1'b1} <<< (FIR_COEFF_FRAC_BITS-1));
            if (acc >= 0) begin
                rounded_mag = acc[ACC_WIDTH-1:0] + round_offset[ACC_WIDTH-1:0];
                shifted = $signed(rounded_mag >> FIR_COEFF_FRAC_BITS);
            end else begin
    ...
    always_ff @(posedge aclk) begin
        if (!aresetn) begin
            ...
            // suppresses its contents until deterministic data reaches output.
        end else begin
