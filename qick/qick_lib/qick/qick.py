...
class QickSoc(Overlay, QickConfig):
    ...
    def __init__(self, bitfile=None, download=True, no_tproc=False, no_rf=False, force_init_clks=False, clk_output=None, external_clk=None, dac_sample_rates=None, adc_sample_rates=None, **kwargs):
        ...
        if no_tproc:
            ...
            print("QICK initalization is done...")
    ...
    def set_all_clks(self):
        ...
        if self['board'] == 'ZCU111':
            ...
            print("resetting clocks:", lmk_freq)
            print("resetting clocks:", lmx_freq)
            print("LMX is not used")
    ...
    def arm_ddr4_samples(self, ch, n_samples, n_triggers=1, address=0, stride_bytes=None, force_overwrite=False, sample_decim=1, target_rate=None):
        ...
        sample_decim : int
            ...
            0, N, 2N, ... after each trigger. This is sample picking in the
    ...
    def fir_readout_length_for_capture(self, n_samples_1msps, margin=1024):
        ...
        if n_samples_1msps < 1:
            raise ValueError("n_samples_1msps must be positive.")
        if margin < 0:
            raise ValueError("margin must be non-negative.")
        decim = int(self.ddr4_buf.cfg.get('fir_decimation', 300))
        group_delay = int(np.ceil(self.ddr4_buf.cfg.get('fir_group_delay_input_samples', 8677)))
